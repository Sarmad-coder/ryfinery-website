...
export class VerifedEmailComponent implements OnInit {
  ...
  constructor(private router: Router, private activeRoute: ActivatedRoute, private authSrv: AuthService, private userService: UserService) {}
  ...
  ngOnInit(): void {
    ...
    if (!token) {
      ...
      this.authSrv.passwordlessLoginByToken(token).then(
        async (resp: any) => {
